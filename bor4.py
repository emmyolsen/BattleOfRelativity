...
def collision(spaceship, planet):
    ...
    if dist <= planet.r + spaceship.r:
        ...
        """
        if (spaceship.x + spaceship.w) < planet.x:
            spaceship.x -= 200
        if (planet.x + planet.w) < spaceship.x:
            spaceship.x += 200
        if (planet.y + planet.h) < spaceship.y:
            spaceship.y += 200
        if (spaceship.y + spaceship.h) < planet.y:
            spaceship.y -= 200
...
"""
PLAN:
- DIDRIK: fikse posisjonering av spaceship etter kollisjon
- fikse liv og fikse tid på runde to (if lives > 1, start_time = real_time)
        i VERSTE FALL: lage en game over skjerm som vises når tiden er ute
- lyd
- ulm diagram
- kommentere koden
